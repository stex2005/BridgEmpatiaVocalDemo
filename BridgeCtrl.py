...
class Thread_ControlClass(threading.Thread):
    def __init__(self, Bridge, Conf, Coord, Debug=False):
        ...
        #self.temp_EndEff0   = numpy.array([0.33167254,te 0.22277789, 0.04704141, 0.0])
        ...
        while self.running:
            ...
            if self.Conf.CtrlEnable:
                ...
                if not self.Coord.GoToSavedPosMainTrigger:
                    # per controllo step del vocale
                    if self.Conf.CtrlInput == 'vocal':
                        if self.Coord.countSteps >= self.Coord.steps:
                            self.Coord.p0 = [0]*4
                            # print '*** Maximum number of steps reached'
                            # self.Coord.countSteps = 0
                            # print 'Max steps reached'
                        else:
                            self.Coord.countSteps = self.Coord.countSteps+1
                            print self.Coord.countSteps
